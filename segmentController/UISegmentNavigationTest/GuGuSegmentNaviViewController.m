...
-(id)initWithItems:(NSArray*)titleArray andControllers:(NSArray*)controllers
{
    ...
    if (self)
    {
        ...
        if([[tmpArr objectAtIndex:0] isEqualToString:@"6"])
        {
            self.automaticallyAdjustsScrollViewInsets = YES;
            y = 0;
        }
        
        barView = [[GuGuSegmentBarView alloc]initWithFrame:CGRectMake(0, y, self.view.frame.size.width, kBarHeight) andItems:titleArray];
        ...
        contentTable = [[GuGuLandscapeTableView alloc]initWithFrame:CGRectMake(0,  kBarHeight + y, self.view.frame.size.width, self.view.frame.size.height - kBarHeight - y) Array:controllers];
...
#warning 此处的320貌似可以改动 没有兼容版本iOS8 iPhone6 6+  因为在GuGuLandscapeTableView中得XY坐标轴的方向修改了 此处(在横向)取得是offset.y  而不是offset.x  
-(void)scrollOffsetChanged:(CGPoint)offset
{
    int page = (int)offset.y / self.view.frame.size.width ;
    float radio = (float)((int)offset.y % (int)self.view.frame.size.width)/self.view.frame.size.width;
